...
class MyTool {
  constructor(topCSSSelector, targetCSSSelectors, inputCSSSelectors) {
    this.topCSSSelector     = topCSSSelector;     
    this.targetCSSSelectors = targetCSSSelectors; 
    this.inputCSSSelectors  = inputCSSSelectors;  
    this.targetContents = {}; 
  }
  ...
  setupCopyItem(CSSSelector) {
    ...
    $(`${CSSSelector} .copy_article`).each(function (index, element) {
      ...
      $copy_input.tooltip({trigger: 'manual'})
        .on('shown.bs.tooltip', function () { // tooltip
          setTimeout((function () {
            $(this).tooltip('hide');
          }).bind(this), 1500);
        });
